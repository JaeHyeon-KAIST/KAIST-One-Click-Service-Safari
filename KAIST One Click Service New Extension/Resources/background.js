browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "startAuthProcess") {
    ...
    browser.storage.local.get("mailOption", (data) => {
      ...
      browser.tabs.query({active: true, currentWindow: true}, async (tabs) => {
        ...
        const startMailAuthProcess = async () => {
          ...
          browser.tabs.create({url: mailUrl, active: false}, (tab) => {
              ...
              browser.scripting.executeScript({
                      ...
                      if (mailLinks && mailLinks.length > 0) {
                        ...
                        const checkMailContent = async () => {
                          ...
                          if (contentElement) {
                            ...
                            setTimeout(checkMailContent, 500);
                          }
                        };
                ...
                if (authCode && authCode.match(/^\d{6}$/)) {
                  browser.windows.getCurrent((currentWindow) => {
                    browser.tabs.query({active: true, windowId: currentWindow.id}, (tabs) => {
                      if (tabs.length === 0) {
                        console.error("No active tab found.");
                        sendResponse({status: "error", message: "No active tab found."});
                        browser.tabs.remove(mailTabId);
                        return;
                      }
    ...
    browser.storage.local.set({mailOption: message.mailOption}, () => {
